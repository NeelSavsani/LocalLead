...
app = FastAPI(
    title="LocalLead API",
    description="Geo-targeted local business lead discovery, website auditor, CRM & project management backend.",
    version="1.0.0"
)
...
def discover_and_qualify(req: DiscoveryRequest, db: Session = Depends(get_db)):
    """
    Search an area, discover real businesses, perform website audits, score leads,
    and save them to the database.
    """
    results = discover_businesses(req.location, req.radius_km, req.categories or [])
    ...
    for idx, b_data in enumerate(results["businesses"], start=1):
        ...
        a_model = WebsiteAnalysis(
            ...
            opportunity_score=audit_data["opportunity_score"],
            audit_notes=audit_data["audit_notes"]
        )
        db.add(a_model)

        # Calculate Lead Score & Priority
        score, priority = calculate_lead_score(b_data, audit_data)
        ...
        act = LeadActivity(
            ...
            note=f"Discovered via geo-scan at {results['search_location']}. Score: {score} ({priority}).",
...
def download_excel(status: Optional[str] = None, db: Session = Depends(get_db)):
    """Export lead database to an Excel (.xlsx) file download."""
    query = db.query(Lead).join(Business)
    if status and status != "ALL":
        query = query.filter(Lead.status == status)
    ...
    filename = f"LocalLead_Export_{datetime.datetime.now().strftime('%Y%m%d_%H%M')}.xlsx"
    return Response(
        content=excel_bytes,
